...
public class ReservationService {
  ...
  public Reservation createReservation(ReservationRequest dto) {
    Boolean canBook =
        restTemplate.getForObject(
            "http://member-service/{memberId}/can-book",
            Boolean.class,
            dto.getMemberId());
    ...
    RoomDto room =
        restTemplate.getForObject(
            "http://room-service/{roomId}", RoomDto.class, dto.getRoomId());
    ...
    restTemplate.put("http://room-service/{roomId}", off, dto.getRoomId());
  ...
  public void performCancelConfirmed(Reservation r) {
    Long memberId = r.getMemberId();
    r.setStatus(ReservationStatus.CANCELLED);
    reservationRepository.save(r);
    RoomUpdateRequest on = new RoomUpdateRequest();
    on.setAvailable(true);
    restTemplate.put("http://room-service/{roomId}", on, r.getRoomId());
    maybeUnsuspendAfterChange(memberId);
  }

  public void performCompleteConfirmed(Reservation r) {
    Long memberId = r.getMemberId();
  ...
  private void maybeSuspendAfterBooking(Long memberId) {
    ...
    try {
      MemberDto m =
          restTemplate.getForObject(
              "http://member-service/{id}", MemberDto.class, memberId);
